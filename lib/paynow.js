...
async function createPayment({ amount, method, phone, reference, description }) {
  ...
  // For initiate, status is empty string.
  // Body fields: id, reference, amount, additionalinfo, returnurl, resulturl, status, method, phone
  const amountStr = parseFloat(amount).toFixed(2);
  const additionalinfo = description || 'Stigma top-up';
  const hashValues = [id, reference || 'ref', amountStr, additionalinfo, returnUrl, resultUrl, ''];
...
function verifyUpdate(params) {
  // Paynow update POST hash: sha512(id + reference + amount + additionalinfo + paynowreference + status, key)
  const key = process.env.PAYNOW_INTEGRATION_KEY;
  if (!key) return false;
